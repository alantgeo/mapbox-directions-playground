    ...
    <v-main>
      ...
      <div id="panel">
        <v-list dense>
          ...
          <template v-if="waypoints.length">
            <v-list-item v-for="(waypoint, index) in waypoints" :key="index">
              ...
              <v-list-item-content>
                <v-list-item-title>Waypoint {{ index + 1 }}</v-list-item-title>
                <v-list-item-subtitle>{{ waypoint }}</v-list-item-subtitle>
              </v-list-item-content>
              <v-list-item-action>
        ...
        <v-layout row wrap ma-4>
          <v-flex xs12>
            <v-btn @click="optimizeWaypoints"> Optimize Waypoints </v-btn>
          </v-flex>
        </v-layout>
        <v-layout row wrap ma-4>
          <v-flex xs12>
          ...
          <v-flex xs12>
            <v-btn-toggle v-model="display" mandatory>
              <v-btn text value="none"> None </v-btn>
              <v-btn text value="speed"> Speed </v-btn>
              <v-btn text value="congestion"> Congestion </v-btn>
            </v-btn-toggle>
          </v-flex>
        </v-layout>
        <v-layout row wrap ma-4>
            ...
            <v-btn-toggle v-model="mode" mandatory>
              <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                  <v-btn text v-on="on" value="driving-traffic">
              ...
              <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                  <v-btn text v-on="on" value="driving">
                    <v-icon>directions_car</v-icon>
                  </v-btn>
                </template>
                <span>Driving</span>
              ...
              <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                  <v-btn text v-on="on" value="cycling">
              ...
              <v-tooltip bottom>
                <template v-slot:activator="{ on }">
                  <v-btn text v-on="on" value="walking">
        ...
        <v-layout row wrap ma-4>
          ...
          <v-flex xs12>
            <v-checkbox
              v-model="avoidToll"
              :disabled="mode === 'walking' || mode === 'cycling'"
              label="Avoid Tolls"
            ></v-checkbox>
          </v-flex>
          <v-flex xs12>
...
mapboxgl.accessToken =
  urlParams.get("access_token") || localStorage.getItem("MapboxAccessToken");
...
const MapboxOptimization = require("@mapbox/mapbox-sdk/services/optimization");
const optimizationService = MapboxOptimization({
  accessToken: mapboxgl.accessToken,
});
...
export default {
  ...
  mounted() {
    ...
    const originSVG = document.createElementNS(
    ...
    const originSVGCircle = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "circle"
    );
  ...
  watch: {
    ...
    origin: function (origin) {
      if (origin) {
        this.originMarker.setLngLat(origin).addTo(this.map);
  ...
  computed: {
    googleMapsUrl: function () {
      ...
      if (this.origin && this.destination) {
        return `https://www.google.com.au/maps/dir/${this.origin
          .slice()
          .reverse()}/${this.destination.slice().reverse()}/data=${
          dataOptions[this.mode]
        }`;
      } else {
        return "";
      }
  ...
  methods: {
    ...
    formatDuration: (durationInSeconds) => {
      ...
      const displayMinutes = Math.round(
        durationInMinutes - Math.floor(durationInMinutes / 60) * 60
      );
    ...
    optimizeWaypoints: function () {
      optimizationService
        .getOptimization({
          profile: this.mode,
          waypoints: [this.origin, ...this.waypoints, this.destination].map(
            (waypoint) => {
              return {
        ...
        .then(
          (res) => {
            ...
            const optimization = res.body;
            optimization.waypoints
              .slice(1, optimization.waypoints.length - 1)
              .map(() => {
                this.waypointswaypoint.waypoint_index;
              });
